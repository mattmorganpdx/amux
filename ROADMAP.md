...
- [ ] **libnotify is called from socket handler threads** — `showDesktopNotification` invokes `notify_notification_new`/`show` directly from whichever handler thread served the request, rather than dispatching to the GTK main thread like every other UI touchpoint. Pre-existing; not addressed while fixing the store's own races (`src/notification_store.zig`)
