...
- [ ] **Segfault in libghostty on shutdown after pane churn** — reproducible 3/3 on both this build and the one before the handler split, so pre-existing and not caused by it. Create and close workspaces with splits, then close the window: teardown logs `Gtk-CRITICAL: gtk_window_set_title: assertion 'GTK_IS_WINDOW (window)' failed`, saves the session a second time after surfaces are already unrealised, then faults inside `libghostty.so` at a varying address (the signature of a use-after-free). Repro harness: `scratchpad/trial.sh`
