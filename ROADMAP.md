...
**Code quality**
- [ ] **Shutdown use-after-free** — `Server.deinit` frees `registry`, `claude_session_store` and `socket_path` and destroys `self` while detached `handleClient` threads may still hold `self`; `stop()` joins only the accept thread. Pre-existing and not worsened by the detach fix (unjoined threads ran to completion regardless), but it needs an in-flight request counter drained before teardown (`src/socket/server.zig`)
...
- [ ] **`notification_store` thread-safety unaudited** — reached from handler threads; not yet checked for the same class of race fixed elsewhere
