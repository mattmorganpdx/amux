...
- [ ] **No explicit socket permissions** — `bind()` leaves the socket at whatever the umask allows (observed `0775`, so others cannot connect since that needs write); an explicit `0600` would be correct (`src/socket/server.zig`)
...
- [x] **Segfault in libghostty on shutdown after pane churn** — was 3/3 reproducible; now 8/8 clean exits. Three separate defects, localised by marking each cleanup step in `main()`:
  - **`ghostty_app_free` raced live Ghostty threads.** The fault was inside `app.deinit()`. `ghostty_surface_free` does not join a surface's renderer/io threads — measured directly, closing panes never lowers the process thread count — so they outlive every surface amux creates and keep referencing app-level state that `ghostty_app_free` tears down. There is no point at which that free is safe, so the exit path skips it and lets the OS reclaim the memory, the same policy as the socket server's drain timeout.
...
- [ ] **libghostty never joins a surface's renderer/io threads** — `ghostty_surface_free` leaves them running, so each surface amux has ever created costs ~6 threads and their stacks for the process lifetime; measured by counting `/proc/<pid>/task` across pane split/close cycles. This is what forces the exit path to skip `ghostty_app_free`, and it caps how long a heavily-churned session can run. Needs a fix in the Ghostty fork, not here
